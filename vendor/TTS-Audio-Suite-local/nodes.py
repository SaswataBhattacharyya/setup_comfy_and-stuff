...
try:
    scene_splitter_module = load_node_module("scene_splitter_node", "audio/scene_splitter_node.py")
    SceneSplitterNode = scene_splitter_module.SceneSplitterNode
    SCENE_SPLITTER_AVAILABLE = True
except Exception as e:
    print(f"❌ Scene Splitter failed: {e}")
    SCENE_SPLITTER_AVAILABLE = False

try:
    scene_edit_driver_module = load_node_module("scene_edit_driver_node", "audio/scene_edit_driver_node.py")
    SceneEditDriverNode = scene_edit_driver_module.SceneEditDriverNode
    SCENE_EDIT_DRIVER_AVAILABLE = True
except Exception as e:
    print(f"❌ Scene Edit Driver failed: {e}")
    SCENE_EDIT_DRIVER_AVAILABLE = False

try:
    scene_concat_module = load_node_module("scene_concat_node", "audio/scene_concat_node.py")
    SceneConcatNode = scene_concat_module.SceneConcatNode
    SCENE_CONCAT_AVAILABLE = True
except Exception as e:
    print(f"❌ Scene Concat failed: {e}")
    SCENE_CONCAT_AVAILABLE = False
...
if SCENE_SPLITTER_AVAILABLE:
    NODE_CLASS_MAPPINGS["SceneSplitterNode"] = SceneSplitterNode
    NODE_DISPLAY_NAME_MAPPINGS["SceneSplitterNode"] = "✂️ Scene Splitter"

if SCENE_EDIT_DRIVER_AVAILABLE:
    NODE_CLASS_MAPPINGS["SceneEditDriverNode"] = SceneEditDriverNode
    NODE_DISPLAY_NAME_MAPPINGS["SceneEditDriverNode"] = "🎛️ Scene Edit Driver"

if SCENE_CONCAT_AVAILABLE:
    NODE_CLASS_MAPPINGS["SceneConcatNode"] = SceneConcatNode
    NODE_DISPLAY_NAME_MAPPINGS["SceneConcatNode"] = "🧩 Scene Concat"
