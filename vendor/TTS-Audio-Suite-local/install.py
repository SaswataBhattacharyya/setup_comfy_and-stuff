...
class TTSAudioInstaller:
    ...
    def _probe_import_in_subprocess(self, module_name: str, display_name: str) -> tuple[bool, str]:
        """Import a module in a clean subprocess so native library failures surface cleanly."""
        probe_code = (
            "import importlib, traceback\n"
            f"module_name = {module_name!r}\n"
            "try:\n"
            "    module = importlib.import_module(module_name)\n"
            "    print(getattr(module, '__version__', 'imported'))\n"
            "except Exception:\n"
            "    traceback.print_exc()\n"
            "    raise\n"
        )
        ...
        error_text = (result.stderr.strip() or result.stdout.strip() or f"{display_name} probe failed").strip()
    ...
    def validate_installation(self):
        ...
        for module_name, display_name in critical_imports:
            try:
                ok, details = self._probe_import_in_subprocess(module_name, display_name)
                if ok:
                    self.log(f"{display_name}: OK", "SUCCESS")
                else:
                    validation_errors.append(f"{display_name}: {details}")
                    self.log(f"{display_name}: FAILED - {details}", "ERROR")
            except Exception as e:
